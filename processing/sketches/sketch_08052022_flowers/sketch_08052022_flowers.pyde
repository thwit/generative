...
def draw3():
    ...
    for _ in range(5):
        ...
        v = PVector(pwidth / 2 + random(-25, 25), pheight)
        loop_length = int(random(4,8))
        for i in range(loop_length):
            ...
            pen.noFill()
            pen.stroke(strk_col)
            v_ = PVector(v.x + random(-25, 25), v.y - random(50, 150))
            
            pen.line(v, v_)
            
            if random(1) < 0.25 and i < loop_length - 2:
                for dir in [-1, 1]:
                    t = random(1)
                    x = lerp(v.x, v_.x, t)
                    y = lerp(v.y, v_.y, t)
                    v_leaf = PVector(x, y)
                    v_leaf_ = PVector(x + dir * random(50, 125), y - random(75, 150))
                
                    pen.line(v_leaf, v_leaf_)
        ...
        pen.circle(PVector(v.x, v.y - r), r)
