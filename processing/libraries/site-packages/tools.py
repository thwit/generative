...
# Check if a line a-b and c-d intersects
...
def save_image(pg, seed, palette_id=None):
    ...
    pg.save(filename)
    pg.save('latest.png')
    print('Image saved to ' + filename + ' and latest.png')
    

def save_image_name(pg, filename):
...
def get_color_palette(id_=None):
    try:
        json_data = open('Q:\\git\\generative\\processing\\libraries\\site-packages\\named-palettes.json', 'rb',)
    except:
        json_data = open('C:\\Users\\thoma\\git\\generative\\processing\\libraries\\site-packages\\named-palettes.json', 'rb',)
