...
def to_panto_a4(points, draw_width, draw_height, center=False, upscale=True):
    ...
    if center:
        x_offset = (rob_width - max(filter(lambda n: not isinstance(n, int), x))) / 2
        y_offset = (rob_height - min(filter(lambda n: not isinstance(n, int), y))) / 2
...
def save_image(pg, seed):
    filename =  'images/' + datetime.datetime.now().strftime('%d%b%Y-%H%M%S') + '_seed_' + str(seed) + '.png'
    pg.save(filename)
    pg.save('latest.png')
