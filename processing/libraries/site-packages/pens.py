...
class PenBasic(object):
    ...
    def arc(self, c, w, h, start_angle, end_angle, mode=OPEN):
        if self._fill:
            self.filler.arc(c, w, h, start_angle, end_angle, mode, self.fill_col)

        if self._stroke:
            self.stroke(self.stroke_col)
            self.strokeWeight(self.stroke_weight)
            self.pg.arc(c.x, c.y, w, h, start_angle, end_angle, mode)
    ...
    def shape_mix(self, vs):
        if self._stroke or self._fill:
            if self._stroke:
                self.stroke(self.stroke_col)
            else:
                self.pg.noStroke()
            if self._fill:
                self.pg.fill(*self.fill_col)
            ...
            for i, v in enumerate(vs):
                if random(1) < 0.35 and i < len(vs) - 1:
                    self.pg.bezierVertex(v.x, v.y, v.x, v.y, vs[i+1].x, vs[i+1].y)
                else:
                    self.pg.vertex(v.x, v.y)

            
            self.pg.vertex(vs[-1].x, vs[-1].y)
...
class PenBrush(PenBasic):
    def __init__(self, pg, filler):
        ...
        self.wiggle = 1 * self._clean * 1
        self.start_drag = 10 * self._clean * 1
        self.end_drag = 4 * self._clean * 1
        self.start_jiggle = 0.15 * self._clean * 1
    ...
    def set_config(self):
        ...
        self.start_jiggle = 0.15 * self._clean

        self.base_alpha = 25
        self.random_alpha = 5 * self._clean
        self.random_brightness = 15 * self._clean
        self.random_saturation = 5 * self._clean
        self.random_hue = 5 * self._clean
        self.random_strokeweight = 1 * self._clean
    ...
    def line(self, v1, v2, d=10):
        ...
        for _ in range(self.number_parallel):
            ...
            startjig = random(1 - self.start_jiggle, 1 + self.start_jiggle)
            self.line2(v1_ + PVector(d / 2 * startjig, d / 2 * startjig), v2_)

        self.stroke(self.col)
        self.strokeWeight(self.sw)

    def line2(self, a, b, c=None, d=None):
        if c is None:
            steps = int((a - b).mag() / 8)
        else:
            steps = int((a - d).mag() / 16)
        ...
        for i in range(0, steps + 1):
            ...
            if c is None:
                x = lerp(a.x, b.x, t) + randomGaussian() * self.wiggle
                y = lerp(a.y, b.y, t) + randomGaussian() * self.wiggle
            else:
                x = curvePoint(a.x, b.x, c.x, d.x, t) + randomGaussian() * self.wiggle
                y = curvePoint(a.y, b.y, c.y, d.y, t) + randomGaussian() * self.wiggle
...
class PenBrush2(PenBasic):
    def __init__(self, pg, filler):
        super(PenBrush2, self).__init__(pg, filler)

    def line(self, v1, v2, d=15):


        u = v2 - v1
        u_ = v2 - v1
        u.normalize().rotate(radians(90))
        u_.normalize()
        # for each bristle
        for _ in range(10):
    ...
    def line2(self, v1, v2):
        ...
        while t < 1:
            ...
            self.circle(PVector(x, y), noise(x * 0.03, y*0.03) * 10)
