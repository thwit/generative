...
class Rect(object):
    ...
    def circle_pack(self, col):
        self.circle_pack = CirclePack(self.x, self.y, self.w, self.h, 100, 2, 100, col)
        self.circle_pack.generate()

    def draw(self, pen):
        if self.circle_pack is not None:
            self.circle_pack.draw(pen)

        pen.stroke(self.strk_col)

        if self.fill_col is None:
            pen.noFill()
        else:
            pen.fill(self.fill_col)
...
class Mondrian:
    ...
    def panto(self, pen):
        coords = []
        #pen = pens.PenBasic(None, None)

        for rect in self.rects:
            #rect.draw(pen)
            v1 = PVector(rect.x, rect.y)
            v2 = PVector(rect.x, rect.y + rect.h)
            v3 = PVector(rect.x + rect.w, rect.y + rect.h)
            v4 = PVector(rect.x + rect.w, rect.y)
            coords.append([v1, v2, v3, v4])
