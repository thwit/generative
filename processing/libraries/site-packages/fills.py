...
class ScannerFill(BasicFill):
    ...
    def rect(self, ul, w, h, col):
        ...
        for x in range(int(ul.x), int(ul.x+w)+1, 3):
            self.pen.line(PVector(x, ul.y), PVector(x, ul.y+h))

        #for y in range(int(ul.y), int(ul.y+h)+1, 3):
        #   self.pen.line(PVector(ul.x, y + random(-3, 3)), PVector(ul.x+w, y + random(-3, 3)))
    ...
    def circle(self, c, r, col):
        ...
        self.pen.noFill()
        n = 300
        points = [(c.x + cos(2 * PI / n * x) * r, c.y + sin(2 * PI / n * x) * r) for x in range(0, n+1)]
