...
class BasicFill(object):
    ...
    def arc(self, c, w, h, start_angle, end_angle, mode, col):
        self.pg.noStroke()
        self.pg.fill(*col)
...
class LineFill(BasicFill):
    ...
    def rect(self, ul, w, h, col):
        ...
        for _ in range(15000):
            v1 = PVector(random(ul.x - self.margin * w, ul.y + (1 + self.margin) * w),
                         random(ul.y - self.margin * h, ul.y + (1 + self.margin) * h))
            v2 = PVector(random(ul.x - self.margin * w, ul.y + (1 + self.margin) * w),
                         random(ul.y - self.margin * h, ul.y + (1 + self.margin) * h))
    ...
    def circle(self, c, r, col):
        self.pen.strokeWeight(1)
        self.pen.stroke((col[0], col[1], col[2], 20))
...
class CurveFill(BasicFill):
    def __init__(self, pg):
        super(CurveFill, self).__init__(pg)
        self.pen = pens.PenBasic(pg, self)
        self.lum = 10

    def rect(self, ul, w, h, col):
        self.pen.strokeWeight(1)
        self.pen.stroke((col[0], col[1], col[2], self.lum))
        self.pen.noFill()
        ...
        for _ in range(n):
            ...
            for v__ in [v, v_]:
                ...
                if x_or_y < 0.5:
                    if r < 0.5:
                        v__.x = ul.x + randomGaussian() * 0.5
                    else:
                        v__.x = ul.x + w + randomGaussian() * 0.5

                    v__.y = random(ul.y, ul.y + h) + randomGaussian() * 0.5

                else:
                    if r < 0.5:
                        v__.y = ul.y + randomGaussian() * 0.5
                    else:
                        v__.y = ul.y + h + randomGaussian() * 0.5

                    v__.x = random(ul.x, ul.x + w) + randomGaussian() * 0.5

                # Uncomment for soft edges
                # __v.x = random(ul.x, ul.x + w)
                # __v.y = random(ul.y, ul.y + h)
    ...
    def circle(self, c, r, col):
        ...
        for _ in range(n):
            ...
            # First line: No 'edge' visible
            # Second line: 'Edge' visible
            # r_ = r * sqrt(random(1))
            r_ = r

            # calculating coordinates
            v.x = r_ * cos(alpha) + c.x + randomGaussian()
            v.y = r_ * sin(alpha) + c.y + randomGaussian() * 0.5
            ...
            # calculating coordinates
            v_.x = r_ * cos(alpha) + c.x + randomGaussian() * 0.5
            v_.y = r_ * sin(alpha) + c.y + randomGaussian() * 0.5
